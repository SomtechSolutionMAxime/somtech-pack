...
**L'occurrence, et son coût** *(2026-09-19)* : un orchestrateur a inscrit à son registre, sur un fait rapporté, que *« le merge ne passait plus par les orchestrateurs »*. **C'était faux dans sa direction** — le clic avait été **délégué aux** orchestrateurs et aux chefs, pas retiré. Sur cette lecture, **il s'apprêtait à retirer à un chef d'équipe gelé un ordre parfaitement juste** : exiger les deux verdicts de revue sur la tête finale, qui est **l'une des conditions que la délégation exige**. La source était un fichier du poste, **lisible en une commande**.

⚠️ **Ce qui l'a sauvé n'est pas sa méthode : c'est que celui qui lui avait rapporté le fait s'est rétracté de lui-même** — il avait amalgamé deux faits de même direction apparente, la délégation du dirigeant du 15/09 et un titre local à un autre chantier du 02/09. *Une correction rendue par celui à qui elle ne profite pas est ce qui a tenu ici ; on ne bâtit pas un métier là-dessus.*

**La réserve, et elle vaut dans l'autre sens** : cette même délégation avait d'abord été dite à **un seul** orchestrateur, puis relayée. **Trois orchestrateurs l'ont inscrite en `proposed` et non `accepted` tant qu'elle ne venait pas du dirigeant lui-même — ils avaient raison.** La source opposable est le **texte des permissions du poste**, pas le relais qui le rapporte.
