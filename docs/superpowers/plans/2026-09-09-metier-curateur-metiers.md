...
- Modify : `ligne-directe/bin/ligne-directe.js` — **repérer par contenu, pas par numéro de ligne** : le bloc `else if (geste === 'orchestrateur')` (l. **475** au 2026-09-20 ; c'était 428-437 à la rédaction — `#346` a fait grossir le fichier), l'aide en tête (l. **20**, c'était 8-9) et le bloc d'usage vers l. 102-113.
...
- [ ] **Step 3 : verbe** dans `bin/ligne-directe.js` : `else if (geste === 'curateur') { … }` jumeau du bloc `orchestrateur` (**l. 475 au 2026-09-20**, 428-437 à la rédaction — chercher `geste === 'orchestrateur'`) ; aide **l. 20** et un bloc l.113 « `curateur <nom>` — pose le lieu d'un curateur des métiers ». Vert.
