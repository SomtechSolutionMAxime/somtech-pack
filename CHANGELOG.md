...
- **Une boîte occupée est délivrée en dix secondes au lieu de cinq minutes** (T-20260818-0076, PR #285) — **mesuré sur le poste réel** : `livrer.js` vers une session au repos dont la boîte portait un texte immobile, **302,4 s avant** (10 h 46 min 33 → 10 h 51 min 36, `delivre:true`, boîte vidée), **10,4 / 10,5 / 10,6 s après** sur trois envois consécutifs. Le geste lui-même n'a jamais été lent : la touche d'envoi nue vide une boîte en quatre secondes. **Ce qui coûtait cinq minutes était l'attente qui la précède.**
- **La fenêtre d'immobilité existait en DEUX exemplaires, et un seul était annoncé.** Dix secondes dans `herdr.js` — le chemin de la parole du dirigeant —, cinq minutes dans `bin/livrer.js`, chacune écrite chez son appelant. Le lot `T-20260818-0049` a réglé la première et annoncé « dix secondes » sans dire de quel chemin il parlait ; un coordonnateur a mesuré l'autre et trouvé 300 secondes. **Elle vit désormais auprès du geste qu'elle règle**, dans `delivrance.js`, et les deux chemins la lisent au même endroit.
- **Devant un texte COLLÉ, on n'attend plus rien** — il est arrivé d'un seul coup, personne n'a les doigts dessus, il n'y a rien à observer. C'était le cas exact du signalement : une boîte portant `[Pasted text #33]`, identique sur sept relevés, attendue cinq minutes. La règle (`fenetreDImmobilite`) est celle qui existait déjà sur le chemin du dirigeant ; elle sert maintenant les deux.
- **Ni l'avis ni le refus ne chiffrent une attente qui n'a pas eu lieu.** L'avis au destinataire arrondissait à la minute — écrit quand la seule fenêtre valait cinq minutes — et rendait donc « resté immobile pendant les **0 min** où je l'ai observée » à quelqu'un dont on venait de soumettre le texte en son nom. **Le chemin du dirigeant produisait déjà cette phrase**, sans qu'aucun essai la regarde. Il dit maintenant des secondes quand il en compte, et devant un collage il dit pourquoi il n'a pas observé.

### Technique

- **⚠️ Pourquoi 582 essais étaient verts sur un chemin qui mettait cinq minutes.** Le banc empruntait le vrai binaire, la vraie boucle, le vrai faux-herdr — et posait `LIVRAISON_IMMOBILITE_MS: '5'` sur chaque essai, c'est-à-dire **remplaçait exactement la grandeur qui était fausse**. Un banc qui règle ce qu'il éprouve n'éprouve que son réglage. Trois essais neufs lancent désormais le binaire **sans ce réglage**, avec un plafond qui les tue s'ils dépassent : ils rougissent à 25 s sur `origin/main`, ils rendent 0,4 s (collage) et 10,7 s (texte tapé) après.
- **Les deux chiffres.** Ce que le lot **attrape** : 302,4 s → 10,4-10,6 s, soit **29× plus court**, sur le même poste, boîte vidée à chaque fois. Ce qu'il **coûte** : la garde du brouillon vivant a été rejouée sur le poste — texte modifié pendant la fenêtre, **refus nommé à 6,2 s, rien soumis, texte intact** (« le texte a BOUGÉ entre mes deux lectures »). **971 essais, 0 échec** sur les deux paquets.
- **Six secondes, et le chiffre sort d'une mesure.** Le critère est un budget de bout en bout ; le reste du chemin — relire l'écran, livrer, constater la prise — a été chronométré à **2,4 s puis 4,3 s sur le même poste à trente secondes d'intervalle**. Une fenêtre de dix secondes rendait 14,3 s mesurés : sous le critère, avec sept dixièmes de marge. **Une garde qui tient à 5 % près mesure la charge de la machine autant que le code.** Six secondes laissent au reste le double de son pire coût.
- **Ce qui protège le brouillon humain n'est pas la longueur de l'attente.** L'exigence d'origine — « il ne couvre pas *a tapé la moitié puis est parti* » — reste tenue par ce que `delivrerLaBoite` **relit** avant de soumettre : un texte qui a bougé d'un caractère, un dialogue, un écran illisible sont refusés. La fenêtre ne fait que donner de quoi voir. La garde d'essai qui exigeait « au moins cinq minutes » a été **reclassée par sa fonction** — elle porte désormais les deux bords : la fenêtre observe (elle n'est pas nulle) **et** elle tient dans le budget.
- **Dix mutations, dix rouges, zéro survivante** — dont deux qui ont d'abord SURVÉCU et ont fait écrire deux gardes de plus : rendre la fenêtre du texte tapé devant un collage tenait dans le budget de 15 s sans que rien ne rougisse, et faisait raconter à l'avis une observation qui n'avait pas eu lieu.
- **Deux passes de revue** — portail et fond, mutations faites sur copie hors dépôt.
