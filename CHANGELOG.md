...
- **La version du pack ne se lit plus dans `VERSION`** (`T-20260819-0009`) — le `CLAUDE.md` désignait ce fichier comme la source, alors que `publish.yml` l'aligne sur le tag **dans le paquet**, sans jamais recommiter. Mesuré le 2026-08-19 : `VERSION` disait `1.64.0`, le tag et le paquet publié `1.75.0`. La phrase était fausse pour **toute session Claude Code** qui lit le dépôt.
...
- **Le refus est éprouvé sur ce qu'il DIT, pas sur le fait qu'il dise quelque chose** (`T-20260819-0009`) — une garde qui vérifie la présence d'un texte se désarme en y collant la phrase d'une autre branche. La suite exige un **geste nommé au passé**, refuse toute formule de non-tentative — le conditionnel compris, « la touche aurait déjà été pressée » dit qu'il ne s'est rien passé — et impose trois refus **deux à deux distincts**. Éprouvé par mutation, **un point à la fois**, baseline de la copie hors dépôt soustraite : six mutations (retirer chaque phrase, les échanger, et deux textes qui **affirment l'inverse** tout en portant les bons mots-clés), **zéro survivante**.
- **Une garde se juge sur deux chiffres, pas un** — ce qu'elle attrape *et* ce qu'elle refuse à tort. Mesuré : **11 leurres sur 11** attrapés (phrase vide, texte quelconque, négation, conditionnel, futur, phrase qui ne parle que de la boîte, mensonge habillé des bons mots) et **0 refus à tort sur 13** formulations honnêtes différentes. Une première version exigeait la rédaction de son auteur et en refusait 6 sur 8 — dont deux à cause d'un `\b` placé après une lettre accentuée, qui ne marque aucune frontière en JavaScript.
...
- **Et l'un de ces deux essais était vert pour la mauvaise raison** — trouvé en le mutant, pas en le relisant : chercher « refus » dans tout le `stderr` était satisfait par le rapport de herdr lui-même (« a refusé : no state change observed »), jamais par la prose éprouvée. On isole désormais la ligne d'aveu.
