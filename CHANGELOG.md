...
- **Hook `Stop` scribe des tâches — l'orchestrateur écrit ses tickets au ServiceDesk au fil de l'eau, et lit sa prochaine tâche** (`T-20260925-0080`, `D-20260925-0003` pt. 6). À chaque fin de tour, la garde lit le dernier message de l'agent : un bloc ```` ```taches ```` (verbes `ouvrir`/`en-cours`/`fait`/`attend`) déclenche les écritures correspondantes sous la demande déclarée dans `.demande` du lieu, puis rend la prochaine tâche ouverte par `{"decision":"block","reason":…}` — sauf `attend: dirigeant`, qui écrit sans relancer. **Silence strict** si aucun bloc n'est présent (zéro appel réseau) ; tout le reste (bloc mal formé, ServiceDesk injoignable, ticket appartenant à une autre demande, comptes divergents à la lecture de la suite) **refuse en le nommant**, jamais n'écrit à moitié — TOUT est validé avant la première écriture. `cli/src/metier/rendu.js` (`commandeDeHook`) apprend la forme `Stop`, dont le contrat de sortie diffère de `PreToolUse` : une garde absente/cassée refuse (`decision:block`) SAUF si `stop_hook_active` est vrai, auquel cas elle laisse s'arrêter (`systemMessage`) pour ne jamais bloquer l'arrêt à l'infini. **Garde ① — plafond de relances/heure**, paramètre `SOMTECH_SCRIBE_RELANCES_PAR_HEURE` (jamais une constante en dur, arbitrage B, ADR-022 `proposed`) posé à 30 dans `metier/orchestrateur/classement.json` — mesuré sur 38 jours de fins de tour réelles de `batiscan` (pic 25/h, N=30 → 0 refus à tort) ; N absent/invalide → aucune relance, et le hook le dit. `tickets list` ignore silencieusement son filtre `demand_id` (mesuré, `T-20260925-0088`) : la suite pagine sans ce filtre, filtre côté client, et recoupe le compte trouvé à `direct_ticket_count` — un écart rend « non mesuré », jamais une fausse suite. **Deux défauts réels trouvés et corrigés par la revue de fond avant fusion** : ① *bloc cité = vraies écritures* — `extraireBloc` balayait tout le message, donc un exemple cité en prose (« voici la syntaxe : … ») créait un vrai ticket ; seul le bloc qui **termine** le message compte désormais (rien que des blancs après sa clôture) — le refus « deux blocs » devient structurellement impossible sous cette règle et a été retiré plutôt que contourné. ② *rejeu = doublons* — un hook redéclenché sur le même dernier message créait deux tickets identiques ; un **journal PAR ÉTAPE** (empreinte sha256 du bloc + clé stable de chaque étape du plan réussie), gardé dans l'état du lieu, rend désormais un rejeu idempotent (aucune réécriture, et le hook le dit) tout en restant capable de relire la suite. **Un 3ᵉ défaut réel, trouvé et corrigé par une seconde passe de revue de fond** : un rejeu APRÈS UN ÉCHEC PARTIEL (`ouvrir` réussi, `en-cours` en panne) rejouait tout le plan — y compris le `ouvrir` déjà réussi — parce que l'idempotence ne portait que sur une empreinte de FIN de plan, jamais atteinte sur un échec. Le journal par étape ferme ce trou : chaque écriture réussie est notée immédiatement (rappel `noterEtapeReussie`, appelé avant même de savoir si la suite du plan va réussir), donc le journal enregistré reflète toujours l'état RÉELLEMENT atteint, y compris sur un refus. Un rejeu qui saute des étapes et trouve une suite le dit dans le `reason` du `decision:block`. **4ᵉ passe (revue de fond ter)** : le journal PAR ÉTAPE lui-même n'était exercé par AUCUN témoin de bout en bout — `cli/src/metier/gardes/scribe-taches.js` (module pur) était bien gardé, mais la ligne du FIL MINCE qui persiste réellement le journal sur disque (`gardes/scribe-taches.js`, la fusion `resultat.journalAEnregistrer ?? journal`) pouvait être mutée en `journal: null` sans qu'aucun test ne rougisse — les témoins D2/D3 n'exerçaient que la décision, chaque lancement du fil partait d'un état neuf. Ajout d'un témoin qui lance le VRAI fil mince DEUX FOIS DE SUITE, même fichier d'état, avec un double du ServiceDesk injecté via un second seuil de test (`SOMTECH_SCRIBE_APPELER_TEST`, gardé par le MÊME signal que la cloison d'essais de `ligne-directe/src/cloison.js` — `NODE_TEST_CONTEXT`, posé uniquement par `node --test`, jamais en production) : `tickets.create` n'est appelé qu'une fois au total sur les deux lancements. **Le délai interne vérifié et fermé, pas seulement documenté** : si le minuteur du fil mince tue le process au milieu du plan, `deciderStop` ne rend jamais son `journalAEnregistrer` (le process sort avant que sa promesse ne se résolve) — un nouveau rappel `onEtapeReussie`, injecté et appelé SYNCHRONE après chaque étape réussie (avant même de savoir si la suite du plan va réussir), persiste désormais le journal immédiatement, étape par étape, pendant que le plan tourne encore. Témoin dédié : une étape qui pend indéfiniment (simulant un vrai socket ouvert, pas une promesse nue qui laisserait la boucle d'événements vide et le minuteur `unref()` ne jamais se déclencher) prouve que l'étape précédente, réussie, est déjà sur disque quand le minuteur coupe. **5ᵉ passe** : le SEUIL DE TEST lui-même (le second point d'injection de `gardes/scribe-taches.js`, hors de `transportServiceDesk`) n'était gardé par aucun témoin — deux mutations survivaient : retirer `NODE_TEST_CONTEXT &&` de sa condition, et retirer `typeof mod.appeler === 'function'` sur le module injecté. Les témoins précédents tournaient TOUJOURS sous `node --test`, donc `NODE_TEST_CONTEXT` était TOUJOURS hérité par l'enfant — aucun ne pouvait distinguer « le garde-fou marche » de « il n'existe pas ». Deux témoins ajoutés : l'un retire explicitement `NODE_TEST_CONTEXT` de l'environnement de l'enfant (clé ServiceDesk aussi retirée) et prouve qu'un double journalisant ses appels n'est JAMAIS invoqué (le fil retombe sur le vrai `transportServiceDesk()`, qui rend `null` sans clé — aucun appel réseau tenté) ; l'autre injecte un module dont `appeler` est une chaîne, pas une fonction, et prouve que le refus reste nommé « clé absente » (pas « injoignable », qui serait la trace d'un appel raté PLUS LOIN — la forme du refus est la preuve, pas seulement sa présence). Aucun code n'a changé (le contrôle `typeof` et le garde-fou `NODE_TEST_CONTEXT` étaient déjà corrects) — seule la couverture manquait. **6ᵉ passe** — trois chemins non gardés, trouvés chacun sur un fichier différent : **Z1** — `cli/src/metier/rendu.js`, `plafond_par_heure` mal formé (`NaN`, négatif, zéro, non-numérique) dans le classement pouvait produire une commande portant `SOMTECH_SCRIBE_RELANCES_PAR_HEURE=NaN` ; **choix : le rendu REFUSE le classement** (erreur nommée, même polarité que les budgets L0/L1 — un artefact produit est une garantie, jamais une intention approximative), et la commande elle-même porte une seconde ligne de défense (`> 0`, pas seulement `isFinite` — un plafond négatif était FINI et passait avant ce correctif, bug réel trouvé au passage). **Z2** — `gardes/scribe-taches.js`, `dernierMessageAssistant` : le chemin transcript (sans `last_assistant_message`) n'était exercé par AUCUN témoin ; mesuré sur un vrai transcript du poste (169 messages assistant) — 71 sont du `tool_use` PUR sans aucun texte, 40 portent exactement un bloc texte, aucun n'en porte plus d'un ; **décision, tranchée sur cette mesure : lire le texte du DERNIER message assistant qui EN A**, jamais le premier, jamais un `tool_use` muet — déjà le comportement du code, désormais gardé par un témoin qui construit un transcript réaliste (plusieurs tours, ligne JSON illisible, `tool_use` muet après le vrai dernier texte). **Z3** — `gardes/scribe-taches.js`, `lireEtat` : un fichier d'état corrompu (JSON illisible) ne plantait pas, mais repartait silencieusement d'un état vide — **choix : refus nommé, pas un état vide silencieux**, parce que repartir à vide efface le journal et un rejeu du même bloc risquerait de recréer un ticket déjà ouvert (D3 par une autre porte) ; le refus est PRIORITAIRE sur toute autre validation (bloc malformé compris), et le fichier corrompu n'est JAMAIS réécrit par le hook (sans quoi la preuve de corruption disparaîtrait au tour suivant et le prochain rejeu repartirait silencieusement à vide, exactement le défaut que ce refus existe pour fermer) — un humain le répare ou le supprime. Banc du métier re-baseliné 182 576 → 183 385 → 183 512, marge zéro (inchangé depuis la 2ᵉ passe — les 3ᵉ à 6ᵉ défauts sont purement code/tests, aucun texte de métier n'a bougé). **7ᵉ passe — REJET fondé, reproduit par EXÉCUTION DU VRAI FIL MINCE** : état corrompu (Z3) + `stop_hook_active:true` rendait ENCORE `decision:block` — et un état corrompu empêche le plafond de compter (les horodatages ne se lisent pas d'un fichier illisible) → **boucle sans fin**. Le fil ne passait jamais `stop_hook_active` à la décision. Règle posée, GÉNÉRALE et pas un correctif au cas par cas : **R1** — un bloc absent (ou cité, pas terminal) reste SILENCE, toujours, avant toute lecture d'état, même corrompu (déjà vrai dans le code, désormais gardé par un témoin qui le prouve avec un fichier réellement corrompu sur disque) ; **R2** — TOUT REFUS (bloc malformé, ServiceDesk injoignable, état corrompu, etc.) rendu sous `stop_hook_active:true` NE BLOQUE PLUS : arrêt permis avec un `systemMessage` qui porte la raison (elle a déjà été dite comme block au tour précédent — la répéter n'apprend rien à l'agent et ne sert qu'à boucler) ; une RELANCE légitime (« prochaine tâche ») reste `decision:block` même sous `stop_hook_active`, bornée par le plafond seul — c'est la chaîne voulue (le hook continue de pousser du travail réel après un premier blocage, jusqu'à ce que le plafond morde) ; **R3** — `stop_hook_active` est lu par le fil mince (`gardes/scribe-taches.js`) et passé à `deciderStop`, qui le teste lui-même. `gaterEtEmettre` distingue désormais un « candidat » de REFUS (`gate`, désarmable par R2) d'un « candidat » de RELANCE (`gateRelance`, jamais désarmé par `stop_hook_active`, seulement par le plafond). Témoin de reproduction exacte du défaut, sur le VRAI fil mince (état corrompu réel sur disque, `stop_hook_active:true`) : plus de `decision:block`. **🔴 DÉFAUT RÉEL DE CODE, trouvé en QA CONTRE LE VRAI SERVICEDESK** (`D-20260925-0004`) : `preverifierDemande` appelait `demands get` avec `header:true` — mesuré sur le vrai service, le mode en-tête NE REND PAS `direct_ticket_count` (ni `epic_count`, `work_link_count`), seule la réponse COMPLÈTE les porte. **Conséquence réelle : le hook refusait CHAQUE bloc valide** (« réponse sans les champs attendus ») et n'a jamais pu écrire sur le vrai service, alors que les 93 tests passaient tous — le double du ServiceDesk rendait `direct_ticket_count` même sous `header:true`, plus cohérent que le service réel. Double corrigé pour être CONFORME au réel (`header:true` → objet `demand` sans ces trois champs, comme le vrai) ; `header: true` retiré de l'appel — plus lourd (description, commentaires embarqués), mais c'est la seule forme qui porte le compte dont ce module a besoin. **8ᵉ passe (couverture)** — quatre mutations survivantes, code déjà juste (vérifié par exécution), seule la couverture manquait : **M5** (la plus grave) — un bloc avec SEUL `en-cours:` d'une autre demande n'était isolé par aucun témoin (le seul témoin voisin croisait `en-cours` correct + `fait` fautif, donc `fait` seul suffisait à faire refuser et `en-cours` n'était jamais ce qui faisait la différence) — témoin dédié ajouté, zéro `update` avant refus. **M1** — le filtre du hook `Stop` (`rendu.js`) sans le contrôle de non-vacuité sur `reason` aurait retransmis un `{"decision":"block","reason":""}` tel quel — un block sans texte relancerait l'agent sans lui dire pourquoi. **M2** — l'égalité stricte `stop_hook_active===true` mutée en simple troncature (« truthy ») aurait laissé un hôte sérialisant le champ en chaîne (`"false"`, elle-même vraie en JS) ou en nombre désarmer la commande de repli par accident. **M4** — `trouverProchaineTache` : aucun témoin ne mêlait des candidats AVEC et SANS `sequence_order` — un candidat rangé doit passer avant tout candidat non rangé, quelle que soit sa date de création (les non-rangés se départagent ensuite par `created_at`). Banc du métier inchangé (183512, marge zéro) : aucun texte de métier n'a bougé. 98 tests (`cli/test/metier-garde-scribe-taches.test.js`, `metier-rendu-hook-stop.test.js`, `metier-fil-scribe-taches.test.js`), 34 mutations posées et tuées au total depuis le début du lot (dont 4 lors de cette 8ᵉ passe), plus la vérification dédiée du défaut `header:true` (rouge sur 11 témoins existants avec l'ancien code, confirmant la portée réelle du défaut avant correctif). **9ᵉ passe — DEUX DÉFAUTS RÉELS TROUVÉS EN QA CONTRE LE VRAI SERVICEDESK** (`D-20260925-0004`) : `ouvrir` en réel a créé `T-20260925-0097` ✅, le rejeu sans doublon a fonctionné (« reprise : 1 étape sautée ») ✅, la suite a été rendue ✅ — mais `fait` échouait : « l'écriture fait … commentaire a échoué (réponse sans contenu) ». **D-a** — le hook n'envoyait pas `author_label` à `tickets add_comment`, obligatoire pour une clé sans JWT (mesuré : HTTP 200, `{"error":{"code":-32603,"message":"author_label is required"}}`). Le fil mince (`gardes/scribe-taches.js`) construit désormais ce libellé — le nom de l'agent lu dans `<lieu>/.nom-agent` (une ligne) s'il existe, sinon le nom du dossier du lieu, suffixé « (scribe des tâches) » — et le transmet à `executerEcritures`, qui l'envoie UNIQUEMENT sur `add_comment` (le contrat MCP ne l'exige que là, pas sur `create`/`update`). Double de test rendu conforme au réel : `add_comment` sans `author_label` refuse désormais avec exactement le message mesuré. **D-b** — `transportServiceDesk` (`ligne-directe/src/mandat.js`) jetait « réponse sans contenu » sur une enveloppe JSON-RPC `error` (HTTP 200, PAS une erreur HTTP) et PERDAIT le message du service — la seule information qui dit quoi réparer. Il jette désormais `Error(<error.message>)` quand l'enveloppe porte une erreur avec un message non vide ; le comportement générique (« réponse sans contenu ») reste inchangé pour toute autre réponse sans contenu exploitable, y compris une erreur sans message. Recherche faite : aucun appelant existant (`grep` sur tout le dépôt) ne dépendait du texte « réponse sans contenu » pour ce cas précis. ⚠️ `tickets update` (fermeture) n'a **pas** été mesuré en réel — le double reste au contrat MCP documenté (`author_label` non exigé sur `update`), non contredit par la QA à ce jour ; à confirmer. Chaîne complète (cli + `ligne-directe` + banc métier) rejouée. `cli` : 101 tests (`metier-garde-scribe-taches.test.js`, `metier-rendu-hook-stop.test.js`, `metier-fil-scribe-taches.test.js`) — 3 nouveaux ce tour. `ligne-directe` : 1492 tests (3 nouveaux ce tour, `tests/le-transport-du-mandat-ne-parle-pas-a-la-prod.test.js`). 37 mutations posées et tuées au total depuis le début du lot (dont 3 lors de cette 9ᵉ passe : contrôle `enveloppe.error.message` désarmé, `author_label` retiré de `add_comment`, lecture de `.nom-agent` désarmée). Banc du métier inchangé (183512, marge zéro).
- **`#378` — les orchestrateurs moins volubiles** (`D-20260925-0003` · mandat `T-20260925-0026`, lot 1 ; recalibrée par `T-20260925-0086`, lot 3). « J'ai besoin de toi » seulement sur un message qui attend quelque chose ; une question au dirigeant est une décision (deux options, une reco) ; aucun identifiant technique sur la ligne ; `LU` dès la réception, avec son premier fait ; ronde : tour vide sans message, question déjà posée non re-posée. Recalibrée sur `main` après `#383` : fusion sans conflit, rendu conforme, banc re-baseliné 183 512 → 183 469 (−43, une baisse : le plafond ne voit pas ce qu'on retire), marge zéro.
- **`#379` — demander au chef avant de fermer** (`T-20260925-0015` · mandat `T-20260925-0026`, lot 2 ; recalibrée par `T-20260925-0086`, lot 3). Pouvoir lire l'espace de travail d'un chef d'équipe n'exempte pas l'orchestrateur de lui demander avant de le fermer : la clôture passe par le chef, jamais par une lecture de son état. Recalibrée sur `main` après `#383` et `#378` : 22 conflits, tous issus de 3 blocs dans 2 chapitres (les deux PR avaient raccourci les mêmes paragraphes), résolus bloc par bloc, copies dérivées régénérées ; banc re-baseliné 183 469 → 183 806, marge zéro.
...
- **Banc mensuel `protege`/`prose`/`contredit`/`[non établi]` sur les ADR et STD** (`.claude/skills/orchestrer-chantier/lib/banc-protection-mensuelle.sh` + compagnon `.py`) — rejoue, de façon mécanique et sans appel réseau/MCP, la mesure « ce qui tient par un mécanisme vs ce qui reste de la prose » sur un corpus figé de 67 règles (41 ADR + 24 STD + 2 entrées invitées : le sous-thème `/merge` de STD-030 et le garde-fou `GF-ORC-013`, tous deux nés dans ce même lot 4). Ne rend **jamais** `absent` — une citation introuvable dans ce dépôt rend `[non établi]`, jamais une conclusion sur son inexistence ailleurs (corpus Somcraft connu incomplet). **Témoin positif obligatoire** : les 4 gardes déjà livrées du lot 4 (STD-038, STD-030 §/merge, STD-029, GF-ORC-013) doivent être vues `protege`, mécanisme cité — sinon c'est le banc qui a tort, pas le corpus. Rend l'**écart** avec le passage précédent (`PREMIER_PASSAGE` au premier tour, sinon la liste des ids qui changent de classe), jamais seulement l'état. Échoue bruyamment (rc≠0, zéro ligne de classement) si le corpus est absent/illisible/invalide, ou si l'appelant déclare la source cassée. **Champ `REGLE_<ID>_AUDITE=oui|non`** en sortie, pour chaque règle : `oui` seulement quand le classement vient d'un `mecanisme_connu`/`contredit_par` déclaré et vérifié (les 8 entrées `verifie_a_la_main` du corpus) ; `non` pour l'heuristique générique (les 59 autres), **quel que soit son verdict** — un `protege` de l'heuristique peut être une pure coïncidence de deux fichiers sans rapport (repro réelle : `STD-030`, voir défaut n°4). **Sept défauts réels trouvés et corrigés avant fusion**, un par le chef d'équipe, six par six tours indépendants de revue de fond (sous-agents Sonnet frais, chacun sans mémoire des précédents) :
  1. Le corpus figé et l'état de sortie vivent sous `scripts/`/`.claude/` (les répertoires scannés) et énumèrent chaque id en clair — sans exclusion explicite du corpus/de l'état (fichier exact ET dossier entier, y compris un état périmé jamais référencé par l'appel courant), le banc se « trouvait » lui-même comme citation et rendait `prose` au lieu de `non_etabli` pour la majorité du corpus (58/67 au lieu de 17/67 après correctif).
  2. La classe `protege` cherchait la citation N'IMPORTE OÙ dans le dépôt scanné plutôt que DANS le mécanisme/banc déclaré — un mécanisme vidé de sa logique (fichier gardé, contenu retiré) aurait quand même été vu `protege` tant que la chaîne cherchée traînait ailleurs (ex. un SKILL.md sans rapport). Corrigé en scopant la vérification aux fichiers `chemin`/`banc` déclarés (`grep_fichier`, comme la branche `contredit_par` le faisait déjà).
  3. **Le correctif n°2 restait insuffisant** : il acceptait la citation dans `chemin` OU `banc`, or un fichier de test est nommé d'après ce qu'il teste et cite presque toujours la même chaîne que son mécanisme — QUE LE MÉCANISME FONCTIONNE ENCORE OU NON. Reproduit concrètement sur STD-038 réel du corpus par le 2e tour de revue de fond : `staging-secret-key-gate.sh` vidé à 0 octet restait classé `protege` parce que `test-staging-secret-key-gate.sh` continuait de citer « service_role »/« sb_secret_ ». Corrigé en exigeant la citation **exclusivement** dans `chemin` (l'implémentation réelle) — `banc` doit toujours exister, mais son contenu ne prouve plus rien sur l'état du mécanisme. L'exclusion du défaut n°1 a aussi été durcie pour ne plus dépendre de `__file__` (fragile sous le banc de mutation, qui exécute une copie du script dans un tmpdir) mais de `racine`, toujours fiable.
  4. **L'heuristique générique (les 59 entrées non `verifie_a_la_main`) reproduisait le même défaut que n°2/3, jamais fermé pour elle** : `protege` dès qu'un fichier `lib/`+un fichier `test*.sh` citaient tous deux la chaîne, sans aucun lien réel entre eux ni avec la règle. Trouvé par le 3e tour de revue de fond, **sur une entrée réelle du premier passage déjà livré à Somcraft** : `STD-030` (générique) classé `protege` via `merge-closes-stories.sh` (qui protège en réalité le sous-thème `/merge`, tracé séparément sous `STD-030-MERGE`) croisé avec `test-verifie-brief-chef.sh` — sans rapport. Le docstring promettait déjà que ces classements « ne comptent dans aucun chiffre d'exactitude », mais rien ne le rendait visible dans la sortie ni dans les rapports agrégés. Corrigé en ajoutant le champ `AUDITE` (ci-dessus) plutôt qu'en tentant de fiabiliser l'heuristique elle-même (hors d'atteinte sans analyse sémantique, limite déjà documentée).
  5. **Documentaire (4e tour de revue de fond, agent frais) : le bloc `# SORTIE` de `banc-protection-mensuelle.sh` ne mentionnait pas `AUDITE`**, alors qu'il est émis pour chaque entrée depuis le défaut n°4 — un futur intégrateur lisant ce contrat public en ignorerait l'existence. Corrigé.
  6. **`ECART_PRECEDENT` ne détectait jamais l'apparition ou la disparition d'un id entre deux passages (5e tour de revue de fond, agent frais)** — la boucle ne portait que sur les ids du passage COURANT, et excluait explicitement un id sans équivalent dans l'état précédent (`ancien is not None`). Un ADR/STD retiré du corpus (même s'il était `contredit`) rendait `AUCUN` écart : la pire des deux lectures possibles de « rien n'a changé ». Corrigé en bouclant sur l'UNION des ids des deux passages et en nommant explicitement `<id>:NOUVEAU-><classe>` / `<id>:<classe>->DISPARU`. Même tour : une entrée de corpus malformée (`mecanisme_connu` qui ne serait pas un objet) plantait par traceback Python non documenté plutôt que l'échec bruyant `rc=3` déjà promis pour un corpus invalide — durci en conséquence, même principe que la validation de `entries`.
  7. **Le garde-fou du défaut n°6 restait incomplet (6e tour de revue de fond, agent frais)** : il vérifiait la présence de la clé `id`, pas son TYPE — un `id` numérique ou `null` passait la garde puis faisait planter `sanitiser_cle(rid)`, juste après le seul bloc try/except (qui ne couvrait que l'appel à `classer_entree`), par le même `TypeError` non documenté que le défaut n°6 prétendait fermer. Corrigé en exigeant `id` non vide et de type `str` avant tout usage.
  
  Bancs : 50/50 unitaire (dont 11 tests de régression sur ces défauts, rouge confirmé sans chaque correctif), 14/14 sur le corpus réel — témoin positif OK (qui exige aussi `AUDITE=oui` sur les 4 témoins, pas seulement `protege`), 8/8 entrées vérifiées à la main classées correctement (0 à tort, sur 8 vérifiées / 67 au total), 12/12 mutations tuées (0 survivante, 0 inopérante — un test devenu structurellement inerte par une défense en profondeur ajoutée en cours de route a été retiré et documenté, pas laissé à mentir). Six passes de revue indépendantes de fond, chacune un agent frais sans mémoire des tours précédents : `REJET` aux tours 1 à 6 (défauts n°2 à 7), chacun corrigé avant le tour suivant. Plus une passe portail (Haiku) `RIEN VU`. Premier passage réel (les classes elles-mêmes sont inchangées par ces sept défauts — qui touchaient la fiabilité de la preuve et la robustesse du banc, pas les nombres de ce passage précis) : 5 `protege` **dont 4 audités et 1 par coïncidence de l'heuristique (`STD-030`, à vérifier manuellement)**, 17 `prose`, 4 `contredit`, 41 `non_etabli` — porté au tableau stratégique (Somcraft) avec cette distinction. Les 17 `prose` trouvées ont chacune donné lieu à un ticket ServiceDesk (constat non corrigé, hors-scope de ce lot).
...
- **🔴 La veille de déblocage n'abandonne plus tout agent à sa première pause** (`T-20260921-0077`). Elle concluait `agent-termine` dès que l'agent était `done` sur deux relevés. Or herdr documente `done` comme « il a fini **son tour**, personne n'a regardé le pane » : c'est l'état **normal** d'un agent qui a répondu et attend le message suivant. Mesuré le 2026-09-21 sur un vrai agent à nous : réponse « ok », puis `done` à 8 relevés d'affilée ; la veille posée dessus sortait en quelques secondes. C'est ce qui explique **482 veilles mortes, 101 agents, aucune vivante** — l'agent repartait ensuite sans personne derrière, et se bloquait à la première permission. La confirmation « sur deux relevés » ne corrigeait rien : un agent au repos est stable, les deux relevés concordaient sur le faux.
  - `done` et `idle` suivent désormais **un seul chemin** : le repos, compté, borné, nommé `repos-prolonge` (code 12). **`agent-termine` n'est plus jamais prononcé** : rien à l'état ni à l'écran ne dit « fini » avec certitude. Une veille sur un agent réellement fini part toujours — à la borne de repos, en disant ce qu'elle a vu.
  - **Coupé par une limite d'usage** (« You've hit your session limit… Continuing automatically at … ») : elle **reste** — aucune borne de repos ne court, la reprise est automatique et peut être à des heures — et le dit une fois. Vaut aussi quand l'agent est `blocked` : l'écran de limite n'est plus compté comme « écran non reconnu ».
  - **Le journal que `--list` affiche existe dès la pose** (il ne se créait qu'au premier déblocage : qui suivait le chemin concluait que la veille n'avait rien écrit). La pose annonce **les deux fichiers** (sortie et déblocages) et `--list` affiche aussi `sortie=`.
  - **La retenue sur limite est bornée des deux côtés** (passe portail sur les correctifs eux-mêmes). Une phrase ordinaire qui *cite* « usage limit reached » ne retient plus l'agent au repos : ces deux libellés exigent une heure de reprise sur la même ligne (régression : la veille perdait sa borne `repos-prolonge`). Un vieux message de limite n'est un vestige que dans une fenêtre de mémoire de 20 h (`VD_LIMITE_MEMOIRE`) — l'heure de reprise n'a pas de date, une 2ᵉ coupure réelle de même heure ne s'oublie plus. `done`/`idle` remettent à zéro le compteur de retenue (relevés consécutifs seulement), et le journal porte le vrai numéro de relevé. **Compromis assumé de la sonde, ses deux faces** : une phrase ordinaire citée *avec* une heure ou « reset(s) » retient encore la veille jusqu'à `tours-epuises` (borné) ; une vraie coupure au libellé long (`…usage limit reached|<epoch>`, heure à la ligne suivante, « until 15:00 ») n'est pas vue et retombe sur la borne de repos — l'état d'avant ce lot. La borne de repos est le livrable, la sonde un bonus ; suite : `T-20260921-0110` (parser l'heure, ou retirer la sonde).
  - Banc : scénarios 50–52 (l'agent passe par `done` puis reçoit le message suivant ; agent réellement fini ; limite d'usage en `done`, `idle`, `blocked` ; contre-épreuves « sans limite » et « parler de limite »). Les scénarios 6, 9, 10, 47b, 48b, 48f **encodaient le défaut** (`done` ⇒ fin) et ont été réécrits.
...
- 🔴 **La revue de fond a trouvé que le CÂBLAGE n'était gardé nulle part.** Les calculs étaient tous justes et tous éprouvés — et **quatre mutations indépendantes du branchement restaient vertes** : vider la route `case 'version'` de `cli.js` (la commande ne fait plus rien et rend **rc=0**, exactement ce que le module interdit), inverser deux champs dans l'appel de `setup.js` (le marqueur porterait le vestige du dépôt), figer le registre à `null` (il ne serait plus **jamais** consulté), et accepter une sortie npm corrompue. *Un correctif parfait qui n'est jamais atteint ne corrige rien* — la même forme qui, sur `T-20260815-0013`, avait laissé passer un bloc qui n'exécutait jamais son `source`.
- **`cli/test/version-poste-cablage.test.js` — 10 cas qui empruntent le CHEMIN RÉEL** : `run(['version', …])` et `run(['setup', …])`, sortie capturée et code de retour vérifié, sur des postes jetables. Dont l'aller-retour complet : `setup` écrit, la commande relit.
- ⚠️ **Le cas de l'inversion a dû être rendu DISCRIMINANT** : dans le dépôt, `cli/package.json` et `pack.json` portent le **même numéro** — inverser les deux champs y était indiscernable. Le banc fabrique donc un payload dont la version du contenu diffère.
- 🔴 **Le GARDE-FOU de la couture n'était gardé par rien — et le banc tenait par un hasard d'environnement.** Écrire `if (stub)` au lieu de `if (stub !== undefined)` rend la chaîne vide *falsy* : le code part alors interroger le **vrai registre**. Les 58 tests restaient verts. Pire : le cas « registre injoignable » ne passait que parce que le `$HOME` factice du banc prive `npm` de l'auth du poste — l'appel échouait **pour une autre raison que celle voulue**, et retombait sur `null` par accident. *Sur un runner où l'auth passerait, le même test serait faux.* ⚠️ Et une assertion sur la valeur rendue ne l'aurait **pas** vu : le `catch` générique avale l'exception et rend `null` quand même. **C'est l'APPEL qu'on mesure, pas son retour** — avec son symétrique, sans lequel la garde serait satisfaite en ne prenant jamais le chemin réel.
- **La couture d'injection du registre ne soustrait pas le chemin réel à l'épreuve** : `interrogerRegistre` est éprouvé avec un `exec` injecté — on vérifie que c'est bien **`npm view` sur le registre du pack** qui est appelé, qu'une sortie non-semver ne devient pas une version, et qu'un `npm` qui échoue rend `null` sans exploser.
- **`cli/test/version-poste.test.js` — 13 cas**, dont un **aller-retour réel** : ce que `setup` écrit, la commande le relit.
- **`scripts/tests/test-mutations-version-poste.sh` — 32 assertions, 30 défauts réintroduits, 0 survivante**, dans la CI, et **`cli.js` comme `setup.js` sont désormais dans sa portée**. L'instrument **refuse une mutation sans effet** et le prouve sur lui-même ; le chiffre annoncé ici est comparé au compte **réellement joué**, jamais au plancher.
...
- **La revue de fond a rendu `correctifs`, et les trois défauts étaient réels — vérifiés par le fait avant d'être corrigés.** ① 🔴 **La validation du format était un `case` glob, pas une expression régulière.** En shell, `[0-9]*` veut dire « un chiffre puis **n'importe quoi** » : `v1.2.3.4`, `v1.2.3abc`, `v1a.2.3` passaient — et surtout **`v1.9*.0`, dont l'étoile est un motif que `ls-remote` interprète**. Le refus nommait alors **un tag qui n'existe pas, avec le sha d'un autre** — l'inverse exact de ce que le ticket demande. La règle est désormais écrite **une seule fois** (`MD_SEMVER_MOTIF`), ancrée des deux côtés, et partagée par le filtre de flux et le validateur. ② **Les zéros de tête sont interdits** : `v01.02.03` et `v1.2.3` se lisent comme le même numéro et sont **deux refs différentes** — l'une libre pendant que l'autre est prise, c'est-à-dire `T-20260815-0013` par une autre porte. Mesuré avant de resserrer : aucun des 144 tags du serveur n'en porte. ③ **La vérification n'était pas « juste avant de taguer »** malgré le titre de la section : entre elle et `git tag` il y avait la **confirmation humaine**, d'une durée non bornée — exactement la fenêtre du 2026-08-15. Elle est **rejouée dans le bloc qui tague**, ce qui couvre aussi le cas où l'utilisateur propose un autre numéro.
...
- 🔴 **Le correctif ne taguait JAMAIS — et c'est le banc qui le cachait.** Le bloc « Execution » ne sourcait pas la lib : la **Confirmation** qui le précède est un **arrêt réel** pour attendre l'humain, et le shell n'y survit pas. En usage réel, `md_version_libre` était **introuvable** — `command not found`, le `||` se déclenchait, et le message parlait de *disponibilité* alors que la fonction était simplement absente. **À chaque tentative, pas seulement en cas de course.** Le banc ne pouvait pas le voir : il **injectait `source <lib>` en tête** avant de jouer le bloc, et corrigeait donc en silence ce que le texte ne faisait pas. *C'est « le joint qui permet d'éprouver soustrait à l'épreuve » (vague 2B, règle 6), commis dans le banc écrit pour ça.* Le bloc **se source lui-même**, le banc le joue **verbatim depuis la racine**, et la mutation qui retire ce `source` rougit.
- 🔴 **Le texte du skill n'est plus DEVINÉ : il est EXÉCUTÉ.** Deux approximations successives sont tombées. La garde a d'abord cherché le **nom** de la fonction — un appel nu, code de retour jeté, la satisfaisait. Elle a ensuite cherché un **opérateur** (`||`, `&&`, `if`) — mais `|| true` et `&& echo "on continue"` sont du shell ordinaire qui porte l'opérateur **sans rien gouverner**, tandis qu'un gate légitime écrit en `case "$rc"` était rejeté à tort. *Gouverner est une propriété d'**exécution** ; aucune expression régulière ne la tranche.* Le bloc est donc **extrait du skill et joué** contre un `git` fabriqué, et le banc observe si `git tag` est atteint sur `LIBRE`, `PRIS` et `serveur injoignable`. Le texte du skill rejoint ainsi la famille de la lib : **quelque chose qu'on fait tourner**. Trois contre-épreuves — `|| true`, `&&` inversé, gate placé après la pose — le prouvent.
- **Un second tour de fond a trouvé le dernier défaut, et il était dans la GARDE DE LA GARDE.** Le contrôle exigeait que `md_version_libre` soit **cité** dans le bloc qui tague — pas qu'il le **gouverne**. Un appel **nu** (l'appel présent, son code de retour jeté, `git tag` derrière) satisfaisait le `grep` et **ne protégeait rien** : les 175 assertions restaient vertes. Le contrôle exige maintenant un **gate** (`||`, `if`, `&&`) et vérifie qu'il vient **avant** la pose. *« La garde vérifie le contenu, pas le fait » — le motif 1 du brief, commis dans la garde écrite pour empêcher la régression.*
- ⚠️ **Une séparation de `stderr` a été écrite puis RETIRÉE — troisième ligne ingardable de ce lot.** Elle et le filtre sur la ref exacte se recouvraient : la mutation qui refusionnait les flux **survivait**, parce que le filtre écarte déjà toute ligne qui n'est pas la ref demandée. On garde **celle qui peut rougir**, et un relais qui écrit sur `stderr` en réussissant l'éprouve.
...
- **`CLAUDE.md` du dépôt — le même fait y était redit, et n'était pas corrigé.** « La version du pack se lit sur le tag git — `git tag --sort=-v:refname | head -1` » : la même prescription locale, dans le document que **toute session lit**. Corrigée, et **gardée au même titre que le skill** — une garde bornée au skill aurait laissé l'ancien geste prescrit ailleurs.
...
- **La revue de fond a rendu `correctifs`, avec trois survivantes — toutes dans la couche du TEXTE, aucune dans la lib.** ① `PROCHAINE … DISTANT %s LOCAL %s` : la garde vérifiait la **présence** des étiquettes, jamais la **valeur** qu'elles portent — permuter les deux laissait la suite verte, et le récapitulatif du skill (« DISTANT fait foi ») aurait été inversé en silence. La garde compare maintenant la **ligne entière**, à l'endroit du banc où local et distant diffèrent. ② Le skill prescrit qu'une mesure impossible n'autorise **aucune** suppression : rien ne le gardait. ③ **La plus grave** : le geste destructif de l'étape 7 était borné aux branches `merged` par une phrase que rien ne tenait — la reformuler en « toute branche listée » rendait `unmerged` et `INDETERMINE` supprimables, suite verte. Les trois sont gardées et contre-éprouvées.
- **Une SECONDE revue de fond, sur le lot déjà corrigé, a rendu `correctifs` et trouvé quatre défauts de plus — dont un dans la lib.** ① **`md_max_semver` débordait sur le champ voisin** : la clé pondérée `maj*1000000 + min*1000 + pat` donne exactement le même nombre à `v1.0.1000` et `v1.1.0`, et à `v1.1000.0` et `v2.0.0` — le comparateur rendait **le mauvais tag**, sans erreur, sur un dépôt qu'aucun scénario n'approchait. Remplacé par une comparaison **champ par champ**. ② et ③ La garde du geste destructif cherchait `merged` dans un **voisinage de huit lignes** : une phrase-leurre la satisfaisait, et sept lignes d'explication légitime la faisaient **rougir à tort**. Elle remonte maintenant à la **puce qui gouverne** le bloc, et une contre-épreuve vérifie aussi **l'absence de faux positif**. ④ La garde des chiffres interrogeait le **plancher** — une valeur entretenue à la main — au lieu du compte **réellement joué** : abaisser les deux ensemble laissait tout vert. *C'est, dans le banc, exactement la forme que le chef d'équipe venait de vivre en se croyant nommé parce qu'un `pane get` lui rendait son étiquette d'écran.*
- **Un troisième tour de fond a fermé le dernier défaut — et il était dans la garde elle-même.** Le contrôle du geste destructif remontait à « la dernière puce `- **` vue, **où qu'elle soit** » : réécrire la liste en liste **numérotée** faisait sortir le scan de la section, qui retombait sur une puce sans rapport plus haut dans le document. **Rouge à tort** sur un texte dont le fond était intact ; **vert à tort** sur un geste réellement débridé, dès que cette puce lointaine mentionnait `merged`. Le scan accepte maintenant **toute forme d'item** et **oublie l'item courant à chaque titre** — il ne traverse plus une frontière de section. Les deux sens sont contre-éprouvés. *La même forme, une troisième fois : interroger un voisin au lieu de l'objet.*
- **Un chiffre de couverture rouillé se lit comme frais** : le CHANGELOG annonçait encore 52 et 28 assertions, mesurés avant six scénarios. Corrigé — et **gardé** : les nombres annoncés ici doivent égaler les planchers des deux bancs, sinon la suite rougit.
...
- **`scripts/tests/test-merge-mesure-distante.sh` — 118 assertions, dans la CI** (les bancs de `.claude/skills/merge/tests/` n'y tournent pas). Chaque scénario est construit pour **diverger** : un dépôt dont le local et le distant concordent ne peut rien prouver.
- **`scripts/tests/test-mutations-merge-mesure-distante.sh` — 65 assertions, 39 défauts réintroduits, 0 survivante**, dont l'**accord skill ↔ lib**. L'instrument **refuse une mutation sans effet** et le prouve sur lui-même. La première ronde tuait 13 sur 13 ; une seconde ronde, non fixée d'avance, a rendu **dix survivantes** — neuf closes, et la dixième a révélé l'inverse : deux nettoyages que **rien ne pouvait faire rougir**, donc retirés plutôt que gardés.
...
- **Mutation — 15 points, un à la fois** *(muter en groupe cache une survivante)*. **14 rouges, 1 mutant équivalent déclaré**, zéro survivante réelle. Le point prescrit — faire pointer la commande sur `git ls-files` au lieu du disque — en fait rougir **sept**. La contre-mesure vérifie d'abord que la **fixture discrimine** : la même fixture, sondée par l'index git, doit **basculer au faux**. Sans elle, le test du disque pourrait être vert pour une mauvaise raison.
- 🔑 **Le premier banc disait « 7 rouges, zéro survivante » — c'était vrai, et ça ne prouvait pas ce qu'on croyait.** Il ne mutait que le MOTEUR. La passe de fond a muté la **PORTE** et trouvé **quatre survivants** : `--role` silencieusement ignoré, `--json` réduit à un geste vide, le code de sortie forcé à 1, et le **dispatch `agent ou-naitre` débranché** — les quatre passaient sans un seul rouge. *Les deux étages étaient justes séparément ; **la ligne qui les relie** n'était gardée par rien.* Onze assertions de porte les ferment, dont une sur la **jointure** elle-même et une sur l'**aide** — une sous-commande que l'aide ne nomme pas ne « rend pas l'écart visible sans qu'on le cherche ». **« Zéro survivante » ne couvre jamais que ce qui était gardé : c'est le dénominateur qui trompait, pas le résultat.**
- **Un lien symbolique vers un dépôt ne le compte plus deux fois** — l'identité est le chemin RÉEL, pas le chemin écrit (relevé en revue : la fonction promettait « sans doublon » et ne tenait sur les chemins écrits à l'identique). *Compter deux fois le même dépôt rendrait « 16 sur 15 » et ferait douter du compte entier.* Une mesure ratée vaut « distinct » : on ne fusionne pas deux entrées sur un `realpath` qui a échoué.
- **Et la contre-mesure a trouvé deux survivants de plus, après le premier correctif** — c'est le motif qui se répète, pas un oubli isolé : **`-h` que l'aide PROMETTAIT sans qu'aucun test ne l'appelle** *(le retirer répondait « ✗ Option inconnue : -h » à qui suivait le texte de la commande)*, et **un `version.json` valide mais MUET sur le champ `version`** *(le test existant couvrait le JSON illisible, jamais le JSON lisible dont le champ manque — un lot y aurait affiché une version devinée)*. Fermés.
- **Un mutant est ÉQUIVALENT, et il est déclaré plutôt qu'escamoté** : retirer `statSync(...).isDirectory()` de la population ne change rien. **Mesuré, pas raisonné** — sur un fichier ordinaire, un lien symbolique cassé et un lien vers un vrai dépôt, les deux branches concordent (`existsSync(<fichier>/.somtech-pack/version.json)` ne peut pas être vrai). *La garde reste, par défense ; on ne bourre pas la suite d'un test qui ne peut pas rougir.*
