...
- **Banc mensuel `protege`/`prose`/`contredit`/`[non établi]` sur les ADR et STD** (`.claude/skills/orchestrer-chantier/lib/banc-protection-mensuelle.sh` + compagnon `.py`) — rejoue, de façon mécanique et sans appel réseau/MCP, la mesure « ce qui tient par un mécanisme vs ce qui reste de la prose » sur un corpus figé de 67 règles (41 ADR + 24 STD + 2 entrées invitées : le sous-thème `/merge` de STD-030 et le garde-fou `GF-ORC-013`, tous deux nés dans ce même lot 4). Ne rend **jamais** `absent` — une citation introuvable dans ce dépôt rend `[non établi]`, jamais une conclusion sur son inexistence ailleurs (corpus Somcraft connu incomplet). **Témoin positif obligatoire** : les 4 gardes déjà livrées du lot 4 (STD-038, STD-030 §/merge, STD-029, GF-ORC-013) doivent être vues `protege`, mécanisme cité — sinon c'est le banc qui a tort, pas le corpus. Rend l'**écart** avec le passage précédent (`PREMIER_PASSAGE` au premier tour, sinon la liste des ids qui changent de classe), jamais seulement l'état. Échoue bruyamment (rc≠0, zéro ligne de classement) si le corpus est absent/illisible/invalide, ou si l'appelant déclare la source cassée. **Champ `REGLE_<ID>_AUDITE=oui|non`** en sortie, pour chaque règle : `oui` seulement quand le classement vient d'un `mecanisme_connu`/`contredit_par` déclaré et vérifié (les 8 entrées `verifie_a_la_main` du corpus) ; `non` pour l'heuristique générique (les 59 autres), **quel que soit son verdict** — un `protege` de l'heuristique peut être une pure coïncidence de deux fichiers sans rapport (repro réelle : `STD-030`, voir défaut n°4). **Quatre défauts réels trouvés et corrigés avant fusion**, un par le chef d'équipe, trois par trois tours indépendants de revue de fond (sous-agents Sonnet frais, chacun sans mémoire des précédents) :
  ...
  Bancs : 41/41 unitaire (dont 7 tests de régression sur ces quatre défauts, rouge confirmé sans chaque correctif), 14/14 sur le corpus réel — témoin positif OK (qui exige désormais aussi `AUDITE=oui` sur les 4 témoins, pas seulement `protege`), 8/8 entrées vérifiées à la main classées correctement (0 à tort, sur 8 vérifiées / 67 au total), 11/11 mutations tuées (0 survivante, 0 inopérante — un test devenu structurellement inerte par une défense en profondeur ajoutée en cours de route a été retiré et documenté, pas laissé à mentir). Quatre passes de revue indépendantes : portail (Haiku) `RIEN VU` ; fond (Sonnet, 1er tour) `REJET` (défaut n°2) → corrigé ; fond (Sonnet, 2e tour, agent frais) `REJET` (défaut n°3) → corrigé ; fond (Sonnet, 3e tour, agent frais) `REJET` (défaut n°4) → corrigé. Premier passage réel (les classes elles-mêmes sont inchangées par les défauts n°2-4, qui touchaient la fiabilité de la preuve, pas le nombre — le risque était réel et déjà déclenché sur `STD-030`, seule la visibilité manquait) : 5 `protege` **dont 4 audités et 1 par coïncidence de l'heuristique (`STD-030`, à vérifier manuellement)**, 17 `prose`, 4 `contredit`, 41 `non_etabli` — porté au tableau stratégique (Somcraft) avec cette distinction. Les 17 `prose` trouvées ont chacune donné lieu à un ticket ServiceDesk (constat non corrigé, hors-scope de ce lot).
