...
- **Banc mensuel `protege`/`prose`/`contredit`/`[non établi]` sur les ADR et STD** (`.claude/skills/orchestrer-chantier/lib/banc-protection-mensuelle.sh` + compagnon `.py`) — rejoue, de façon mécanique et sans appel réseau/MCP, la mesure « ce qui tient par un mécanisme vs ce qui reste de la prose » sur un corpus figé de 67 règles (41 ADR + 24 STD + 2 entrées invitées : le sous-thème `/merge` de STD-030 et le garde-fou `GF-ORC-013`, tous deux nés dans ce même lot 4). Ne rend **jamais** `absent` — une citation introuvable dans ce dépôt rend `[non établi]`, jamais une conclusion sur son inexistence ailleurs (corpus Somcraft connu incomplet). **Témoin positif obligatoire** : les 4 gardes déjà livrées du lot 4 (STD-038, STD-030 §/merge, STD-029, GF-ORC-013) doivent être vues `protege`, mécanisme cité — sinon c'est le banc qui a tort, pas le corpus. Rend l'**écart** avec le passage précédent (`PREMIER_PASSAGE` au premier tour, sinon la liste des ids qui changent de classe), jamais seulement l'état. Échoue bruyamment (rc≠0, zéro ligne de classement) si le corpus est absent/illisible/invalide, ou si l'appelant déclare la source cassée. **Deux défauts réels trouvés et corrigés avant fusion**, l'un par le chef d'équipe, l'autre par la revue de fond (sous-agent Sonnet, frais, indépendant) :
  ...
  2. **Plus grave** : la classe `protege` cherchait la citation N'IMPORTE OÙ dans le dépôt scanné plutôt que DANS le mécanisme/banc déclaré — un mécanisme vidé de sa logique (fichier gardé, contenu retiré) aurait quand même été vu `protege` tant que la chaîne cherchée traînait ailleurs (ex. un SKILL.md sans rapport), exactement le silence que ce lot existe pour détecter. Corrigé en scopant la vérification aux fichiers `chemin`/`banc` déclarés (`grep_fichier`, comme la branche `contredit_par` le faisait déjà — l'asymétrie entre les deux branches trahissait l'oubli). L'exclusion du défaut n°1 a aussi été durcie pour ne plus dépendre de `__file__` (fragile sous le banc de mutation, qui exécute une copie du script dans un tmpdir) mais de `racine`, toujours fiable.
  
  Bancs : 38/38 unitaire (dont 4 tests de régression sur ces deux défauts, rouge confirmé sans chaque correctif), 14/14 sur le corpus réel — témoin positif OK, 8/8 entrées vérifiées à la main classées correctement (0 à tort, sur 8 vérifiées / 67 au total — les 59 autres sont classées par une heuristique générique non auditée, documentée comme telle), 10/10 mutations tuées (0 survivante, 0 inopérante). Deux passes de revue indépendantes : portail (Haiku) `RIEN VU`, fond (Sonnet) `REJET` fondé sur le défaut n°2 → corrigé → re-vérifié. Premier passage réel (inchangé par le correctif n°2, qui ne modifiait aucune classification du corpus actuel) : 5 `protege`, 17 `prose`, 4 `contredit`, 41 `non_etabli` — porté au tableau stratégique (Somcraft). Les 17 `prose` trouvées ont chacune donné lieu à un ticket ServiceDesk (constat non corrigé, hors-scope de ce lot).
