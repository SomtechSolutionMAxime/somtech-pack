...
- **Banc mensuel `protege`/`prose`/`contredit`/`[non établi]` sur les ADR et STD** (`.claude/skills/orchestrer-chantier/lib/banc-protection-mensuelle.sh` + compagnon `.py`) — rejoue, de façon mécanique et sans appel réseau/MCP, la mesure « ce qui tient par un mécanisme vs ce qui reste de la prose » sur un corpus figé de 67 règles (41 ADR + 24 STD + 2 entrées invitées : le sous-thème `/merge` de STD-030 et le garde-fou `GF-ORC-013`, tous deux nés dans ce même lot 4). Ne rend **jamais** `absent` — une citation introuvable dans ce dépôt rend `[non établi]`, jamais une conclusion sur son inexistence ailleurs (corpus Somcraft connu incomplet). **Témoin positif obligatoire** : les 4 gardes déjà livrées du lot 4 (STD-038, STD-030 §/merge, STD-029, GF-ORC-013) doivent être vues `protege`, mécanisme cité — sinon c'est le banc qui a tort, pas le corpus. Rend l'**écart** avec le passage précédent (`PREMIER_PASSAGE` au premier tour, sinon la liste des ids qui changent de classe), jamais seulement l'état. Échoue bruyamment (rc≠0, zéro ligne de classement) si le corpus est absent/illisible/invalide, ou si l'appelant déclare la source cassée. **Trois défauts réels trouvés et corrigés avant fusion**, un par le chef d'équipe, deux par deux tours indépendants de revue de fond (sous-agents Sonnet frais, sans mémoire l'un de l'autre) :
  ...
  Bancs : 39/39 unitaire (dont 5 tests de régression sur ces trois défauts, rouge confirmé sans chaque correctif), 14/14 sur le corpus réel — témoin positif OK, 8/8 entrées vérifiées à la main classées correctement (0 à tort, sur 8 vérifiées / 67 au total — les 59 autres sont classées par une heuristique générique non auditée, documentée comme telle), 10/10 mutations tuées (0 survivante, 0 inopérante — un test devenu structurellement inerte par une défense en profondeur ajoutée en cours de route a été retiré et documenté, pas laissé à mentir). Trois passes de revue indépendantes : portail (Haiku) `RIEN VU` ; fond (Sonnet, 1er tour) `REJET` sur le défaut n°2 → corrigé ; fond (Sonnet, 2e tour, agent frais) `REJET` sur le défaut n°3 (le correctif du 1er tour restait insuffisant) → corrigé ; fond (Sonnet, 3e tour, agent frais) `RIEN VU`. Premier passage réel (inchangé par les défauts n°2/3, qui ne modifiaient aucune classification du corpus actuel — le risque était latent, pas déjà déclenché) : 5 `protege`, 17 `prose`, 4 `contredit`, 41 `non_etabli` — porté au tableau stratégique (Somcraft). Les 17 `prose` trouvées ont chacune donné lieu à un ticket ServiceDesk (constat non corrigé, hors-scope de ce lot).
